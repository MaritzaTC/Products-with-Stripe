                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {
                        prices.map(price => (
                            <div key={price.id} className="max-w-sm  border-2  rounded-lg shadow  dark:bg-gray-800 dark:border-gray-700">
                                <img src={price.image} alt={price.productName} class="rounded-t-lg" />
                                <div className="w-full h-40 flex items-center justify-center overflow-hidden">
                                    <h2 className="text-center">{price.productName}</h2>
                                </div>
                                <h2 ><p class="text-center mb-2 text-2xl font-semibold tracking-tight text-gray-900 dark:text-white">Precio: {(price.unit_amount / 100).toFixed(2)} $
                                </p></h2>
